""" Example of defining a 2-layer MLP in Jax """
...
def init_mlp_params(layer_widths: List[int], keys = None):
    ...
    weights = [random.normal(keys[i], (layer_widths[i], layer_widths[i+1]))
            for i in range(len(layer_widths) - 1)]
    biases = [random.normal(keys[i], (layer_widths[i+1],))
            for i in range(len(layer_widths) - 1)]
